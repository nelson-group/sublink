...
import time
import assembly_common as assemb
reload(assemb)
...
def add_masses(snapnum, in_rad):
    ...
    cat_subs = il.groupcat.loadSubhalos(basedir, snapnum, name='fof_subhalo_tab',
    ...
    all_masses = il.snapshot.loadSubset(basedir, snapnum, parttype_stars, name='snap',
            fields=['Masses'])
    all_formtimes = il.snapshot.loadSubset(basedir, snapnum, parttype_stars, name='snap',
            fields=['GFM_StellarFormationTime'])
    if in_rad:
        all_pos = il.snapshot.loadSubset(basedir, snapnum, parttype_stars, name='snap',
    ...
    start = time.time()
    sub_offset, sub_len = assemb.calculate_subhalo_offsets(basedir, snapnum, parttype_stars)
    print('Time: %f s.' % (time.time() - start))
