...
class MortonCoder {
 ...
 public:
  ...
  code_type code(const Point& p) const {
    ...
    s.x = std::min(std::max(0.0f, s.x), float(cells_per_side-1));
    s.y = std::min(std::max(0.0f, s.y), float(cells_per_side-1));
    s.z = std::min(std::max(0.0f, s.z), float(cells_per_side-1));
 ...
 private:
   ...
   */
  static inline code_type spread_bits(code_type x) {
   ...
   */
  static inline code_type interleave(unsigned x, unsigned y, unsigned z) {
   ...
   */
  static inline code_type compact_bits(code_type x) {
   ...
   */
  static inline code_type smear_low_1(code_type c) {
    c |= c >>  1;
    c |= c >>  2;
    c |= c >>  4;
    c |= c >>  8;
   ...
   */
  static inline code_type smear_low_3(code_type c) {
    c |= c >>  3;
    c |= c >>  6;
    c |= c >> 12;
    c |= c >> 24;
